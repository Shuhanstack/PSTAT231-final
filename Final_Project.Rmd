...
##### 4. Report the dimention of election.raw after removing rows with fips = 2000. Provide a reason for excluding them. Please make sure to use the same name election.raw before and after removing those observations. 
```{r}
...
num_of_candidate <- length(nrow(candidate_table))
```
...
poverty_map <- ggplot()+
  geom_polygon(data = states, aes(x = long, y = lat, fill = candidate, group = group), color = "white")+
  geom_point(data = state_loc, aes(x = long, y = lat, size = pct_poverty, color = pct_poverty), alpha = 0.9)+
...
```{r}
coe_var <- unlist(glm.fit)
name_var <- names(trn.cl)
sig_var <- NULL
for(i in 2:28){
  if(coe_var[i]<0.05){
    sig_var <- append(sig_var, name_var[i-1])
  }
}
```

- Variables that are significant are: `r sig_var`. It is not consistent with what we have in decision tree. 
- Interpretation of some of the coefficients of the variables. <br>
...
regu_train_err = calc_error_rate(regu_pred_test, Ytest.cl)
records[3,1] = regu_train_err
records[3,2] = regu_train_err
```
